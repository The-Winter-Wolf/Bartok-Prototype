...
public class Deck : MonoBehaviour
{
    ...
    public void ReadDeck(string deckXMLText) 
    {
        ...
        for (int i=0; i<xDecos.Count; i++) {
            ...
            // Vector3 loc инициализируется как [0,0,0], поэтому необходимо изменить его
        ...
        for (int i=0; i<xCardDefs.Count; i++) {
            ...
            CardDefinition cDef = new CardDefinition(); // Создать экземпляр CardDefinition
    ...
    {
        // Выйти, если это не карта с картинкой
        if (card.def.face == "") {
            return; 
        }
    ...
    private Sprite GetFace(string faceS) 
    {
        foreach (Sprite _tSP in faceSprites) {
            // Если найден спрайт с требуемым именем вернуть его
            if (_tSP.name == faceS) {
                return(_tSP);
            }
        }
    ...
    static public void Shuffle(ref List<Card> oCards) {
        ...
        tCards = new List<Card>();  // Инициализировать временныфй список
        // Повторять, пока не будет перемещены все карты в исходном списке
